        ...
        <div class="logo_ccon1">
            <div class="logo_enter1" style="border:none;float:left;">
                <div class="login_box1 form-group">
                    <span class="login_title1 col-sm-2 control-label">科目等级</span>
                    <select class="use_name form-control input-sm" name="ctype"  style="width:100px">
                        <option value="1" <?php if(($data["ctype"]) == "1"): ?>selected="selected"<?php endif; ?>> 一级科目</option>
                        <option value="2" <?php if(($data["ctype"]) == "2"): ?>selected="selected"<?php endif; ?>> 二级科目</option>
                    </select>
                    <font style="color:#FF0000;line-height:40px;">&nbsp;*</font>
                </div>
                <div class="login_box1 form-group">
                    <span class="login_title1 col-sm-2 control-label" >科目名称</span>
                    <input class="use_name form-control" value="<?php echo ($data["cgname"]); ?>" id="cgname" name='cgname'  type="text" placeholder="合同内容名称" /><font style="color:#FF0000;line-height:40px;">&nbsp;</font>
                </div>

                <div class="nav_tcon1" style="float:left;width:98%;">
                    <input type="hidden" name="id" value="<?php echo ($id); ?>"/>
                    <input type="hidden" id="scgname" value="<?php echo ($data["cgname"]); ?>"/>
...
<script>
    $("#daniel_form").submit(function () {
        if($("#cgname").val() == ''){
            ...
            var name = $("#cgname").val();
            var sname = $("#scgname").val();
            var tr = 1;
            if(name != sname){
                $.ajax({
                    type: "POST",
                    url: "/reimburse/getCatelogname",
                    data: "name="+name,
                    async: false,
